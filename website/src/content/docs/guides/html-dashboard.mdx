...
The `viz.WriteHTML` / `viz.ExportHTML` functions produce a **self-contained interactive HTML dashboard** — a single file with embedded CSS and JavaScript, no external dependencies. Open it in any browser, attach it to a CI artifact, or email it.
...
This runs a data pipeline (fetch, validate, transform, save + retry) and prints the audit report. Add a `viz.ExportHTML` call to see the interactive dashboard.
