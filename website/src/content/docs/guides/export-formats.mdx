...
Diagrams, tables, trees, and the HTML dashboard are provided by the `viz` sub-module:
...
All four diagram functions are also available as `Write*` (to `io.Writer`) and `Write*String` (returns string):

```go
mmd, _ := viz.WriteMermaidString(report)
dot, _ := viz.WriteGraphvizString(report)
```
...
_ = viz.ExportTable(report, "summary.md", output.FormatMarkdown, output.RenderOptions{})
_ = viz.ExportTable(report, "summary.html", output.FormatHTML, output.RenderOptions{})
```
