...
- **Critical path algorithm is duplicated in Go and JS.** The Go implementation (`computeCriticalPath()` in `report_builder.go`) computes the memoized DFS for the `CriticalPathDurationMs` field and `CriticalPath()` method. The JS implementation (`computeCriticalPathSteps()` in `dashboard.js`) mirrors the same algorithm client-side for the graph highlighting toggle. If the Go algorithm changes, the JS won't track automatically. TODO: inject `critical_path_steps` into the report JSON to eliminate this duplication (schema decision pending).
- **`enhanceGraph()` is coupled to daghtml DOM internals.** It reads `dataset.id`, `dataset.source`, `dataset.target` on SVG elements produced by the daghtml SDK's `initDAGGraph()`. If daghtml changes its DOM structure or data attributes, the post-processing (retry badges, critical path, search) breaks silently. The function is idempotent (guarded by `container.dataset.enhanced`), so switching graph tabs multiple times is safe.
...
- **Benchmarks**: `BenchmarkRenderHTML_LargeReport` (1000 steps) + `BenchmarkRenderHTML_SmallReport` (3 steps) + `BenchmarkNDJSONStreamer_{100,1000,10000}Events` (streaming throughput to io.Discard).
