...
`go-output@v0.35.0/go.mod` contains local `replace` directives (`=> ./testhelpers`) that Go
...
job) and in `.goreleaser.yml` `before:` hooks. go-output v0.35.0 still has the same replace
...
types.go           — Domain enums: EventType, Phase, StepStatus, StepRef, FailureReason, flowStatusMap, fromFlowStatus, SchemaVersion, AllStepStatuses, AllEventTypes, AllFailureReasons, classifyFailure
event.go           — Event type (embeds StepRef, carries RunID, FailureReason + Cached fields) + convenience/predicate methods (IsAttemptStart, HasError, IsTimeout, IsCanceled, IsUserError, WasCached)
step.go            — StepInfo type (incl. Cached attribution) + stepCore (shared accumulator for live/replay) + toStepInfo() conversion
...
recorder.go        — Core state machine: event capture, step records, attempt tracking, RunID + StepID counters, FailureReason classification on attempt_end
runid.go           — newRunID(): 128-bit crypto-random hex run identifier
cached.go          — Cache-hit attribution API: MarkCached(ctx) + ErrMarkCachedNoStepContext sentinel + step-context injection (withStepContext) + Recorder.markCached
...
report.go          — WorkflowReport type (carries RunID) + Validate() + sentinel errors (incl. ErrRenderFailed) + query methods + Duration()/Summary()/CriticalPath()/PeakConcurrencySteps() + workflow-level helpers (RetriedStepCount, TotalRetryAttempts, TimedOutSteps, HasWorkflowRetries, HasWorkflowTimeouts) + stepsByName + WriteJSON/WriteNDJSON + ExportJSON/ExportNDJSON + computeWallClockDurationMs
report_builder.go  — BuildReport assembly: step records → sorted StepInfo + aggregates (WorkflowSucceeded, finalizeDenormalized, buildFailureSummary) + computeCriticalPath/computePeakConcurrency/computePeakConcurrencySteps + sortEventsByTime
filter.go          — Report filtering (Filtered, ReportOption, WithStepsByStatus, WithCachedSteps/WithUncachedSteps, etc.)
diff.go            — Diff API: DiffResult/StepDiff (incl. Cached flag), Diff() between reports (step-level + aggregate deltas: CriticalPathDeltaMs, PeakConcurrencyDelta, CachedStepCountDelta, CriticalPathStepsAdded/Removed, CachedStepsAdded/Removed) + shared diffNameMembership set difference
...
ndjson.go          — ReadEvents NDJSON reader + StreamEvents callback-based streaming reader (sentinel errors, enum validation on ingest)
replay.go          — ReplayEvents: reconstruct Report from event stream (uses stepCore from step.go, preserves RunID + assigns StepIDs)
stream.go          — NDJSONStreamer: real-time streaming NDJSON writer (thread-safe OnEvent callback, WithAutoFlush, WithFlushInterval, WithBufferSize, CreateNDJSONStreamer)
...
classify.go        — Error classification: RegisterClassifications() + ErrorClassifications() map sentinel errors → go-error-family Family (back-compat shim; owned sentinels now carry their family intrinsically)
migration.go       — MigrateReport: upgrades older JSON reports to current schema, re-derives all denormalized fields + RedriveReportStatuses helper + ErrMigrationEmptyInput/ErrMigrationMissingVersion sentinels
...
cmd/auditlog/    — CLI binary: info/convert/diff/validate/schema/version subcommands (stdlib flag, no deps)
cmd/genschema/   — JSON Schema generator using invopop/jsonschema reflector (depguard-restricted, invoked by go:generate)
schema/          — report.schema.json: Draft 2020-12 JSON Schema for WorkflowReport (generated by cmd/genschema)
```
...
- **Linter config: `nlreturn` IS enabled** alongside `wsl_v5` (commit `fcddcaf` re-enabled it after a brief removal — an older version of this note claimed it was gone; that was wrong). Both demand a blank line before `return` statements; write returns with a preceding blank line and both stay satisfied. All three modules lint clean under the current config.
- **Cached is attribution, NOT a StepStatus.** `flow.StepStatus` (upstream go-workflow) has no cached concept and can never carry one through Before/AfterStep callbacks — and it shouldn't: a cache hit IS a success. `auditlog.MarkCached(ctx)` (called from inside a step body; the ctx is injected by `Attach`'s BeforeStep via `withStepContext`, which preserves deadlines/cancellation so step timeouts keep working) sets an orthogonal flag: `Event.Cached` on that attempt_end, `StepInfo.Cached` (any-attempt semantics — per-attempt truth lives in the event stream, same precedent as `Error`/`FailureReason`), and the `CachedStepCount` aggregate. When the ctx wasn't injected (auditing disabled, or called outside a step body) it returns `ErrMarkCachedNoStepContext` (Rejection) — consumers should treat it as non-fatal. Cached surfaces everywhere: CSV column, CLI `info` breakdown + `⚡cached` markers, `viz.ColumnCached` table column, `⚡cached` label suffix on all diagram + tree exports (label only, never a color change), dashboard `⚡ cached` badges + "Cached" stat card + "Cached only" filter chip (both static and live), and Diff cached deltas (`CachedStepCountDelta`, `CachedStepsAdded`/`Removed`, `StepDiff.Cached`) for cache-rate regression detection. NDJSON replay round-trips the flag.
...
- **HTML dashboard** (`viz/html_render.go`) uses `go:embed` to embed `viz/dashboard.css` and `viz/dashboard.js` as separate files (proper syntax highlighting + linting). The `renderHTML()` function uses `fmt.Sprintf` on the `htmlTemplate` const (eight `%s` verbs: CSS, version, report JSON, metadata JSON, DAG data JSON, dashboard JS, daghtml graph JS, version). Report data is injected via `<script type="application/json">` tags (never parsed as HTML by the browser), and dynamic content in JS is escaped via the `esc()` function. Strict CSP: `default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'`. The daghtml SDK (`go-output/daghtml` v0.35.0) provides the SVG graph renderer with Sugiyama layout (rank assignment via Kahn's, 4-pass barycenter crossing reduction, median-alignment positioning, pan/zoom/touch/click-highlight) — invoked via `initDAGGraph(containerId, dataScriptId)` in the embedded daghtml JS. `dashboard.js` post-processes the daghtml SVG in `enhanceGraph()` to add: critical path highlighting (toggle button, memoized DFS mirroring Go's `computeCriticalPath`), retry count badges (`↻N` amber circles), and graph search/filter. **`enhanceGraph()` is idempotent** — guarded by `container.dataset.enhanced` to prevent duplicate badges/listeners on repeated tab switches. `viz.BuildTypeMetadata()` in `viz/metadata.go` is the single source of truth for enum display metadata consumed by the JS. JS status colors reference CSS variables (`var(--success)` etc.) — no hardcoded hex duplication between CSS and JS. The diagram export hex colors in `types.go` (`stepStatusMeta`) are separate because diagram formats (Mermaid/DOT/D2) require literal hex values, not CSS variables. Node duration labels use `humanizeMs()` in `daghtml_adapter.go` (compact: `<1ms`, `Nms`, `N.Xs`) while the rest of the dashboard uses `humanizeDuration()` in `dashboard.js` (verbose: `48.8s`, `5m 18s`) — different formats are intentional. Tree root detection logic exists in both `viz/tree.go` (Go, server-side tree export) and `dashboard.js` (client-side tree tab) — this is intentional: Go renders the tree export for static files, JS renders the interactive tab. Golden content test (`TestReport_WriteHTML_GoldenContent` in `viz/html_golden_test.go`) validates structural + semantic content (DOCTYPE, CSP, 5 script tags, 5 tab panels, step names, WorkflowID, RunID, embedded CSS/JS, strict CSP policy, graph enhancement markers) — NOT byte-for-byte comparison, which was retired after breaking 6+ times on whitespace/dependency drift.
- **Error classification is INTRINSIC to owned sentinels.** Every sentinel owned by auditlog is constructed via a [go-error-family](https://github.com/larsartmann/go-error-family) family constructor (`errorfamily.NewRejection/NewCorruption/NewTransient/NewInfrastructure`) and therefore implements the `Classified` interface itself, carrying a stable `"auditlog.*"` code and its Family. `errors.Is` semantics are unchanged (identity + `%w` chains preserved; `ErrFileExists` still unwraps to `ErrExportWriteFailed`). Mapping: **Corruption** (exit 65, not retryable) = `ErrEventCountMismatch` (`auditlog.event_count_mismatch`), `ErrStepCountMismatch`, `ErrStatusDrift`, `ErrCountMismatch` (data integrity violations). **Rejection** (exit 1, not retryable) = `ErrEmpty`, `ErrNoEvents`, `ErrOversizedLine` (these three are go-ndjson's errors — re-exported, classified via registry only), `ErrWorkflowIDPathSep`, `ErrReplayNoEvents`, `ErrMigrationEmptyInput`, `ErrMigrationMissingVersion`, `ErrFileExists` (bad caller input). **Transient** (exit 75, retryable) = `ErrReportLoadFailed`. **Infrastructure** (exit 69, not retryable) = `ErrRenderFailed`, `ErrExportWriteFailed`. Private `errUnknownEventType`/`errUnknownPhase`/`errNilStreamCallback` are Rejection. `classify.go`'s `init()` still registers everything into `DefaultRegistry` (harmless back-compat; only the three go-ndjson re-exports actually need it — they are third-party errors and cannot carry intrinsic families). `ErrorClassifications()` remains the canonical map; a sync test (`TestClassify_IntrinsicClassificationWithoutRegistry`) proves intrinsic classification works on an EMPTY registry and pins the intrinsic families to the map, and `TestErrorClassifications_CodesUniquePerFamily` guards code uniqueness (Is matches on code+family). Sentinel `Error()` strings gained a `[family:code]` prefix (e.g. `[Transient:auditlog.report_load_failed] report load failed: ...`); message text after the prefix is unchanged. All 24 I/O error paths still wrap with sentinels.
- **Error management policy (erraudit-clean).** `erraudit ./...` passes with ZERO violations at DEFAULT flags in all three modules; run it after touching any error path. Wrapping idiom is stdlib dual-wrap `fmt.Errorf("%w: context: %w", Sentinel, cause)` — it keeps BOTH the matchable sentinel and the underlying cause in the chain; do not replace it with single-cause wrappers. `erraudit --enforce-samber-oops` / `--enforce-generic-return` / `--enforce-deferred-close` findings are intentionally NOT adopted: the project does not enforce samber/oops (no oops in this repo or sibling larsartmann libs; the ecosystem classification layer is go-error-family), per-function error types would bloat a public API whose matching contract is sentinel-based (sentinel + family + code already answer every matching question), and `defer file.Close()` is documented Go practice. Blank-identifier error discards are only allowed with an inline `//nolint:erraudit // <reason>` (used for: client-disconnect `w.Write`/`stream.Send`/`stream.Close` in `live/server.go`, hub-assigned-ID defensive parse skip in `live/replay.go`, infallible `crypto/rand.Read` in `runid.go` (Go 1.24+), and fixture-expected `w.Do` failure in `testhelpers.RunWorkflow`). Real close-error handling: `cmd/auditlog convert` propagates output-file Close errors via `errors.Join` (data-loss risk), read-only input files use bare `defer f.Close()`, and `live.Server.Shutdown` surfaces a failed subscriber-buffer drain (`"drain subscriber buffers: ..."`) to the caller.
...
- **erraudit CI gate is credential-gated.** `github.com/larsartmann/erraudit` is a
  **private repo**, so it is NOT resolvable from the public module proxy and CI cannot
  install it anonymously (`go install` falls back to direct git and fails on auth). The CI
  `Error audit` job therefore skips cleanly until a `PRIVATE_REPO_PAT` secret (fine-grained
  PAT, Contents:read on that repo) is added; once present the job authenticates git via
  `insteadOf` + `GOPRIVATE` and runs for real. If erraudit ever goes public, remove the
  gate and the "Authenticate git" step. GitHub restriction to remember: job-level `if`
  cannot read the `secrets` context — the flag lives in job-level `env` and each step
  gates on `env.HAS_ERRAUDIT_CREDENTIALS`.
- **golangci-lint v2 config verify is strict about linter setting keys.** Example from a
  real failure: `goconst.min-length` does not exist — the correct key is `min-len`.
  Run `golangci-lint config verify` locally (v2.13.2) after any `.golangci.yml` change
  before pushing; unknown keys fail CI at the "Verify golangci-lint config" step with exit 3.
...
- 570 test functions across 3 modules (core: 262; viz: 234; live: 74; counted with `rg -c '^func (Test|Example|Benchmark|Fuzz)' -g '*_test.go'` per module) covering: disabled/enabled, success/failure, dependencies, retry, timeout/cancel, skip, concurrent steps, fan-out/fan-in, event ordering, OnEvent callback, **cache-hit attribution** (MarkCached step/event/count, sentinel on foreign/disabled ctx, no cross-step leak, 16-step concurrency, retry-then-cached per-attempt truth, JSON + NDJSON replay round-trip, both filter directions, count-drift validation, migration recompute), **CaptureDAG** (pre-execution DAG population, idempotent, disabled no-op, nil workflow safety, sub-workflow traversal, retry config capture, no events generated), export formats (JSON/NDJSON/D2/table/tree/HTML), streaming NDJSON (NDJSONStreamer real-time, concurrent safety 16 goroutines, auto-flush, **WithFlushInterval bounded-latency**, WithBufferSize, encode-error path, flush/Close error paths, round-trip, workflow integration, 100% stream.go coverage), **StreamEvents callback-based streaming reader** (ordered delivery, nil-callback guard, validate wiring, callback-error propagation, empty/no-events boundaries), **MultiWriter fan-out** (composability with Config.OnEvent, registration order, concurrent safety, nil-receiver/no-callback handling), **FailureReason classification** (known values, AllFailureReasons, classify contract for nil/timeout/canceled/generic errors, Event predicates), report validation, query methods, **workflow-level retry/timeout helpers** (RetriedStepCount, TotalRetryAttempts, TimedOutSteps, HasWorkflowRetries, HasWorkflowTimeouts), filter (combined type+time interaction), diff (step-level + **aggregate deltas: CriticalPathDeltaMs, PeakConcurrencyDelta, CachedStepCountDelta, CriticalPathStepsAdded/Removed, CachedStepsAdded/Removed**), replay, load, diagrams (Mermaid/PlantUML/DOT/D2 + **⚡cached marker across all formats + trees**), diagram direction (TD/LR/BT/RL across 4 formats), table column configuration (12 columns, custom selection, ordering, replayed reports), edge-direction consistency (diagrams vs tree), API symmetry (Write\*String functions in `viz`, Export\* functions in `viz`), high-fan-out peak concurrency, diamond-DAG critical path, CriticalPath() step chain, PeakConcurrencySteps(), HTML from replayed/loaded reports, HTML diamond DAG + high fan-out, HTML structural integrity, HTML golden content validation, HTML determinism, HTML cached-honesty (report JSON flag + badges + chip + stat card pinned), edge cases, error classification (all 15 public sentinels mapped to Family, wrapped error chain classification, custom registry registration, ExitCode/IsRetryable behavior, errors.Is identity preserved), **error-path tests** (failing io.Writer injection into all `viz.Write*` methods, unwritable directories for `viz.Export*`, invalid input for `Load*` — verifying errors.Is matches `ErrRenderFailed`/`ErrExportWriteFailed`/`ErrReportLoadFailed` on all wrapped paths), plus regression tests for fixed bugs (status drift, diff ordering, NDJSON line numbers, WorkflowSucceeded honesty about pending steps, stale error cleared on retry success), **fuzz tests** (diagram special-char injection across Mermaid/PlantUML/DOT/D2; multi-step diagram sanitization with unicode/control-char/keyword-collision seeds; HTML XSS injection with structural integrity checks; Classify adversarial wrapped error chains), **property-based tests** (9 Diff algebra properties: identity, added/removed duality, duration anti-symmetry, status-change symmetry, sorted output, critical-path anti-symmetry, peak-concurrency anti-symmetry, critical-path steps duality, cached anti-symmetry + count/membership consistency — 200 iterations each, deterministic seeds; Classify wrapping-preserves-family through arbitrary depth, Classify identity matches ErrorClassifications map — 200 iterations), and **benchmarks** (runtime overhead: Invocation, Attach, BuildReport, EventsCopy, OnEventCallback, RetryWithAudit, MermaidExport; export rendering: viz.WriteD2/viz.WriteTable/viz.WriteTree/viz.WriteJSON/viz.WriteMermaid on 100-step reports; renderHTML small 3-step + large 1000-step reports; godoc examples: Duration, Filtered, PeakConcurrency, CriticalPathDurationMs, WallClockDurationMs, NewMultiWriter, StreamEvents, Diff, MarkCached).
- Coverage: **95.3%** of statements (auditlog package), **93.4%** viz, **95.9%** live.
...
| Helper                                             | Purpose                                                                                                    |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `NewSucceed`, `NewFail`, `NewFlaky`, `NewSlow`, `NewCached` | Construct test step instances                                                                    |
| `SucceedStep`, `FailStep`, `FlakyStep`, `SlowStep`, `CachedStep` | Test step types exported for direct use in tests                                              |
