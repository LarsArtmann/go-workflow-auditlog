...
event.go           — Event type (embeds StepRef, carries RunID, FailureReason field) + convenience/predicate methods (IsAttemptStart, HasError, IsTimeout, IsCanceled, IsUserError)
...
diff.go            — Diff API: DiffResult/StepDiff, Diff() between reports (step-level + aggregate deltas: CriticalPathDeltaMs, PeakConcurrencyDelta, CriticalPathStepsAdded/Removed)
