...
//nolint:lll // CSS and JS are long-line string constants, not Go code.
...
const htmlCSS = `
  :root {
    ...
    --info: #8ec5d4;
    --info-dim: rgba(142,197,212,0.12);
    --transient: #d49060;
    --transient-dim: rgba(212,144,96,0.12);
    --radius: 8px;
    --transition: 0.2s cubic-bezier(0.4, 0, 0.2, 1);
    --font-mono: ui-monospace, 'Cascadia Code', 'JetBrains Mono', Menlo, Consolas, 'Courier New', monospace;
  ...
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .status-badge.pending { background: rgba(154,141,120,0.12); color: var(--text-muted); }
  .status-badge.running { background: var(--warning-dim); color: var(--warning); }
  .status-badge.succeeded { background: var(--success-dim); color: var(--success); }
  .status-badge.failed { background: var(--error-dim); color: var(--error); cursor: help; }
  .status-badge.canceled { background: var(--transient-dim); color: var(--transient); cursor: help; }
  .status-badge.skipped { background: rgba(74,74,74,0.15); color: var(--text-dim); }
  ...
  .event-badge.attempt_start { background: var(--success-dim); color: var(--success); }
  .event-badge.attempt_end { background: var(--warning-dim); color: var(--warning); }
  ...
  .config-badge.retry { background: var(--accent-dim); color: var(--accent); }
  .config-badge.timeout { background: var(--info-dim); color: var(--info); }
  ...
  .graph-layer-label { fill: var(--text-dim); font-size: 11px; font-family: var(--font-mono); opacity: 0.5; }
  ...
  .scope-tree { font-family: var(--font-mono); font-size: 0.85rem; }
...
const htmlJS = `
	...
	const statusLabels = Object.fromEntries(Object.entries(meta.statuses).map(([k,v]) => [k, v.label]));
	...
	const statusCounts = {};
	report.steps.forEach(s => { if (s.status) statusCounts[s.status] = (statusCounts[s.status] || 0) + 1; });
	const statusOrder = ['succeeded','failed','running','pending','canceled','skipped'];
	document.getElementById('legend').innerHTML = statusOrder.map(k => {
		const count = statusCounts[k] || 0;
		return count > 0 ? '<div class="legend-item"><span class="icon">' + (statusIcons[k]||'') + '</span>' + esc(statusLabels[k]||k) + ' <span style="opacity:0.5">(' + count + ')</span></div>' : '';
	}).join('');
	...
	(function renderWaveform(){
		...
		const minT = Math.min.apply(null,ts), maxT = Math.max.apply(null,ts), range = (maxT-minT)||1;
		const maxDur = Math.max.apply(null, events.filter(function(e){return e.duration_ms;}).map(function(e){return e.duration_ms;}).concat([1]));
		container.innerHTML = events.map(function(e){
			...
			const tip = e.event_type + (e.step_name ? ' ' + e.step_name : '') + (e.phase ? ' ' + e.phase : '') + (e.attempt ? ' attempt ' + e.attempt : '') + (dur>0 ? ' ' + dur.toFixed(2) + 'ms' : '');
	...
	document.getElementById('stats').innerHTML = [
		...
		report.critical_path_duration_ms ? {label:'Critical Path', value:report.critical_path_duration_ms.toFixed(1)+'ms'} : null,
	].filter(Boolean).map(function(s){return '<div class="stat-card'+(s.cls?' '+s.cls:'')+'"><div class="label">'+s.label+'</div><div class="value">'+s.value+'</div></div>';}).join('');
	...
	function switchTab(tab){
		document.querySelectorAll('.tab').forEach(function(t){t.classList.remove('active');t.setAttribute('aria-selected','false');t.setAttribute('tabindex','-1');});
	...
	document.addEventListener('keydown', function(e){
		...
		if (tab) { var btn = document.querySelector('.tab[data-tab="'+tab+'"]'); if (btn) switchTab(btn); }
	});
	...
	function configBadges(s){
		var badges = '';
		if (s.has_retry) badges += '<span class="config-badge retry" title="Max attempts: ' + (s.max_attempts||0) + '">\u{1F501} retry</span> ';
	...
	var allSteps = report.steps.map(function(s){
		var deps = (s.dependencies||[]).map(function(d){return '<span>'+esc(d.step_name)+'</span>';}).join(', ');
		var depsR = (s.dependents||[]).map(function(d){return '<span>'+esc(d.step_name)+'</span>';}).join(', ');
		var errMsg = s.error ? esc(s.error) : '';
		var statusBadge = '<span class="status-badge '+esc(s.status)+'"'+(errMsg?' data-error="'+errMsg+'"':'')+'>'+(statusIcons[s.status]||'')+' '+esc(s.status)+'</span>';
		var hasError = (s.status==='failed'||s.status==='canceled') ? '1' : '0';
		return '<tr data-search="'+esc((s.step_name+' '+(s.step_type||'')+' '+s.status).toLowerCase())+'"'
			+' data-sort-name="'+esc((s.step_name||'').toLowerCase())+'"'
			+' data-sort-type="'+esc(s.step_type||'')+'"'
			+' data-sort-status="'+esc(s.status)+'"'
			+' data-sort-attempts="'+s.attempt_count+'"'
			+' data-sort-duration="'+(s.duration_ms||0)+'"'
			+' data-has-error="'+hasError+'">'
			+'<td class="mono" title="'+(s.started_at?'Started: '+esc(s.started_at):'')+(s.finished_at?'\nFinished: '+esc(s.finished_at):'')+'">'+esc(s.step_name)+'</td>'
			+'<td class="mono" style="color:var(--text-dim)">'+esc(s.step_type||'\u2014')+'</td>'
			+'<td>'+statusBadge+'</td>'
			+'<td>'+s.attempt_count+(s.max_attempts>1?'/'+s.max_attempts:'')+'</td>'
			+'<td>'+(s.duration_ms?s.duration_ms.toFixed(3):'\u2014')+'</td>'
			+'<td class="deps-list">'+(deps||'\u2014')+'</td>'
			+'<td class="deps-list">'+(depsR||'\u2014')+'</td>'
			+'<td>'+configBadges(s)+'</td>'
			+'</tr>';
	});
	...
	function syncStepMoreBar(visibleCount){
		...
		if (!existing && needBar) {
			...
			moreBar.innerHTML = '<button class="chip" id="show-more-step">Showing '+visibleCount+' of '+totalShown+' \u2014 Show all</button>';
	...
	function applyStepView(){
		stepRows.sort(function(a,b){
			var av = a.dataset['sort-'+stepSortKey], bv = b.dataset['sort-'+stepSortKey];
			var cmp;
		...
		stepRows.forEach(function(tr){ stepTbody.appendChild(tr); });
		document.querySelectorAll('#tab-steps th.sortable').forEach(function(th){
			th.classList.remove('sort-asc','sort-desc');
			if (th.dataset.sort === stepSortKey) th.classList.add(stepSortDir===1?'sort-asc':'sort-desc');
		});
		var q = document.getElementById('step-search').value.toLowerCase();
		var errorsOnly = document.getElementById('step-errors-only').getAttribute('aria-pressed')==='true';
	...
	document.querySelectorAll('#tab-steps th.sortable').forEach(function(th){
		th.addEventListener('click', function(){
			...
			if (stepSortKey === key) { stepSortDir *= -1; } else { stepSortKey = key; stepSortDir = 1; }
			applyStepView();
		});
	});
	...
	var allEvents = report.events.map(function(e){
		...
		var evtBadge = '<span class="event-badge '+esc(e.event_type)+'">'+esc(eventLabels[e.event_type]||e.event_type)+'</span>';
		...
		return '<tr data-type="'+esc(e.event_type)+'" class="'+(e.error?'has-error':'')+'">'
			...
			+'<td'+errTip+'>'+(e.error?'<span class="status-badge failed"'+errTip+'>error</span>':'')+'</td>'
			+'</tr>';
	});
	...
	if (evtRows.length > EVT_PAGE_SIZE) {
		evtRows.forEach(function(tr,i){ if (i>=EVT_PAGE_SIZE) tr.style.display='none'; });
		...
		document.getElementById('show-more-evt').addEventListener('click', function(){
			evtExpanded = true;
			evtRows.forEach(function(tr){ tr.style.display=''; });
			evtMoreBar.remove();
		});
	}
	...
	document.getElementById('event-filters').innerHTML = eventTypes.map(function(t){
		return '<button class="chip'+(t==='all'?' active':'')+'" aria-pressed="'+(t==='all'?'true':'false')+'" aria-label="Filter by '+t+'" data-filter="'+t+'">'+(t==='all'?'all':(eventLabels[t]||t))+'</button>';
	}).join('');
	document.querySelectorAll('#event-filters .chip').forEach(function(btn){
		btn.addEventListener('click', function(){
			document.querySelectorAll('#event-filters .chip').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});
	...
	(function renderTree(){
		...
		if (!report.steps || !report.steps.length) {
			container.innerHTML = '<div class="panel-empty"><div class="empty-icon">\u{1F5C2}</div><div class="empty-text">No steps recorded</div></div>';
			return;
		}
		var byName = {};
		report.steps.forEach(function(s){ byName[s.step_name] = s; });
		var roots = report.steps.filter(function(s){ return !s.dependencies || !s.dependencies.length; });
		...
		function renderNode(step, parent){
			...
			hdr.innerHTML = '<button class="toggle" aria-expanded="true" aria-label="Collapse">\u25BE</button>'+
				'<span class="scope-name">'+icon+' '+esc(step.step_name)+'</span>'+
				'<span class="scope-count">'+depCount+' dependent'+(depCount!==1?'s':'')+' \u00B7 '+esc(step.status)+(step.duration_ms?' \u00B7 '+step.duration_ms.toFixed(1)+'ms':'')+'</span>';
			hdr.setAttribute('role','button');
			hdr.setAttribute('tabindex','0');
			div.appendChild(hdr);

			var deps = (step.dependents||[]).map(function(d){ return byName[d.step_name]; }).filter(function(s){ return s && !visited[s.step_name]; });
			if (deps.length) {
				var childDiv = document.createElement('div');
				childDiv.className = 'scope-children';
				deps.forEach(function(c){ renderNode(c, childDiv); });
			...
			hdr.addEventListener('keydown', function(e){ if(e.key==='Enter'||e.key===' '){e.preventDefault();toggleNode();} });
			parent.appendChild(div);
		}

		roots.forEach(function(r){ renderNode(r, container); });
	})();

	// Timeline
	var tlSteps = report.steps
		.filter(function(s){ return s.duration_ms; })
		.sort(function(a,b){ return (b.duration_ms||0)-(a.duration_ms||0); });
	if (!tlSteps.length) {
		document.getElementById('timeline-container').innerHTML = '<div class="panel-empty"><div class="empty-icon">\u23F1</div><div class="empty-text">No timing data recorded</div><div class="empty-hint">Step durations appear here after workflow execution</div></div>';
	} else {
		var tlMax = Math.max.apply(null, tlSteps.map(function(s){return s.duration_ms;}).concat([1]));
		var statusBarColors = {
		...
		document.getElementById('timeline-container').innerHTML = tlSteps.map(function(s){
			...
			return '<div class="timeline-row">'
				+'<div class="timeline-label">'+(icon?icon+' ':'')+esc(s.step_name)+(s.attempt_count>1?' (\u00D7'+s.attempt_count+')':'')+'</div>'
				+'<div class="timeline-track">'
				+'<div class="timeline-bar" style="width:'+pct+'%;background:'+color+'" title="'+(s.duration_ms||0).toFixed(3)+'ms"></div>'
				+'</div>'
	...
	function renderGraph(){
		...
		if (!steps.length) {
			container.innerHTML += '<div class="panel-empty"><div class="empty-icon">\u{1F5C2}</div><div class="empty-text">No steps registered</div><div class="empty-hint">Add steps to the workflow to see the dependency graph</div></div>';
			return;
		}

		var nameMap = {};
		steps.forEach(function(s,i){ nameMap[s.step_name] = i; });

		var statusColors = {
			succeeded:'var(--success)', failed:'var(--error)', running:'var(--warning)',
			pending:'var(--text-muted)', canceled:'var(--transient)', skipped:'var(--text-dim)'
		};
		var nodeH=32, padX=50, padY=50, gapX=24, gapY=56, charW=7.2;

		var nodes = steps.map(function(s,i){
			var icon = statusIcons[s.status]||'';
			var label = (icon?icon+' ':'')+s.step_name;
			var w = Math.max(label.length*charW+20, 80);
			return {
				id:i, name:s.step_name, label:label, status:s.status,
				color: statusColors[s.status]||'var(--accent)',
				w:w, h:nodeH,
				attempt_count:s.attempt_count, duration_ms:s.duration_ms,
				step_type:s.step_type, error:s.error,
				has_retry:s.has_retry, has_timeout:s.has_timeout
			};
		});
		...
		steps.forEach(function(s,i){
			(s.dependencies||[]).forEach(function(d){
				var j = nameMap[d.step_name];
				if (j !== undefined) {
					var ek = j+'->'+i;
					if (!edgeSet[ek]) { edgeSet[ek]=true; edges.push({source:j, target:i}); }
				}
			});
		});

		// Phase 1: Rank assignment (longest path from roots via Kahn's algorithm)
		var inDeg = new Array(nodes.length).fill(0);
		var adj = new Array(nodes.length).fill(null).map(function(){return [];});
		var radj = new Array(nodes.length).fill(null).map(function(){return [];});
		edges.forEach(function(e){ adj[e.source].push(e.target); radj[e.target].push(e.source); inDeg[e.target]++; });

		var rank = new Array(nodes.length).fill(0);
		var queue = [];
		var visited = {};
		nodes.forEach(function(_,i){ if(inDeg[i]===0){queue.push(i);visited[i]=true;} });
		while (queue.length) {
			var v = queue.shift();
			for (var k=0; k<adj[v].length; k++) {
				var u = adj[v][k];
				rank[u] = Math.max(rank[u], rank[v]+1);
				if (--inDeg[u]===0) { queue.push(u); visited[u]=true; }
			}
		}
		nodes.forEach(function(_,i){ if(!visited[i]) rank[i]=0; });

		var maxRank = 0;
		nodes.forEach(function(_,i){ maxRank = Math.max(maxRank, rank[i]); });

		var layers = new Array(maxRank+1).fill(null).map(function(){return [];});
		nodes.forEach(function(_,i){ layers[rank[i]].push(i); });

		// Phase 2: Barycenter ordering (4-pass forward+backward to reduce crossings)
		for (var pass=0; pass<4; pass++) {
			var forward = pass%2===0;
			for (var r = forward?1:layers.length-2; forward?r<layers.length:r>=0; forward?r++:r--) {
				var layer = layers[r];
				var adjLayer = layers[forward?r-1:r+1];
				if (!adjLayer) continue;
				var posMap = {};
				adjLayer.forEach(function(u,j){ posMap[u]=j; });
				var bary = layer.map(function(v){
					var neighbors = forward ? radj[v] : adj[v];
					if (!neighbors.length) return layer.indexOf(v);
					var positions = neighbors.map(function(n){ return posMap[n]!==undefined ? posMap[n] : 0; });
					return positions.reduce(function(a,b){return a+b;},0)/positions.length;
				});
				var idx = layer.map(function(_,i){return i;});
				idx.sort(function(a,b){ return bary[a]-bary[b]; });
				layers[r] = idx.map(function(i){ return layer[i]; });
			}
		}

		// Phase 3: Centered positions, then median-alignment to straighten edges
		var pos = new Array(nodes.length).fill(0);
		layers.forEach(function(layer){
			var totalW = layer.reduce(function(s,v){return s+nodes[v].w;},0)+(layer.length-1)*gapX;
			var x = -totalW/2;
			layer.forEach(function(v){ pos[v]=x+nodes[v].w/2; x+=nodes[v].w+gapX; });
		});
		for (var r=1; r<layers.length; r++) {
			layers[r].forEach(function(v){
				var preds = radj[v];
				if (preds.length===0) return;
				var sorted = preds.map(function(p){return pos[p];}).sort(function(a,b){return a-b;});
				var mid = sorted[Math.floor(sorted.length/2)];
				var ci = layers[r].indexOf(v);
				var minL = ci>0 ? pos[layers[r][ci-1]]+nodes[layers[r][ci-1]].w/2+gapX : -Infinity;
				var maxR = ci<layers[r].length-1 ? pos[layers[r][ci+1]]-nodes[layers[r][ci+1]].w/2-gapX : Infinity;
		...
		layers.forEach(function(layer){
			...
			var left = Math.min.apply(null, layer.map(function(v){return pos[v]-nodes[v].w/2;}));
			var right = Math.max.apply(null, layer.map(function(v){return pos[v]+nodes[v].w/2;}));
		...
		[{id:'arrow',fill:'var(--border-active)'},{id:'arrow-hl',fill:'var(--accent)'}].forEach(function(m){
		...
		edges.forEach(function(e){
			var sx = pos[e.source], sy = rank[e.source]*(nodeH+gapY)+nodeH;
			var tx = pos[e.target], ty = rank[e.target]*(nodeH+gapY);
		...
		nodes.forEach(function(n,i){
			var cx = pos[i], cy = rank[i]*(nodeH+gapY);
			var g = document.createElementNS(ns,'g');
		...
		function zoomAt(cx,cy,factor){
			var nw = Math.max(50, vb.w*factor), nh = Math.max(30, vb.h*factor);
		...
		svg.addEventListener('touchstart', function(e){
			...
			if (e.touches.length===1) {
				...
				touchDrag = {pinch:Math.hypot(dx,dy), vw:vb.w, vh:vb.h, cx:vb.x+vb.w/2, cy:vb.y+vb.h/2};
			}
		}, {passive:false});
		...
		svg.addEventListener('touchend', function(){ touchDrag=null; });

		document.getElementById('graph-zoom-in').addEventListener('click', function(){ zoomAt(vb.x+vb.w/2, vb.y+vb.h/2, 1/1.4); });
		document.getElementById('graph-zoom-out').addEventListener('click', function(){ zoomAt(vb.x+vb.w/2, vb.y+vb.h/2, 1.4); });
		document.getElementById('graph-fit').addEventListener('click', function(){ vb={x:-svgW/2,y:-padY,w:svgW,h:svgH}; applyVB(); });
		...
		function clearHL(){
			...
			edgeEls.forEach(function(g){ g.classList.remove('highlighted','dimmed'); g.querySelector('path').setAttribute('marker-end','url(#arrow)'); });
			selected = null;
		}
		Object.keys(nodeEls).forEach(function(id){
			...
			g.addEventListener('click', function(e){
				...
				selected = nid;
				var connected = {}; connected[nid] = true;
				edges.forEach(function(e){
					if (e.source===nid||e.target===nid) { connected[e.source]=true; connected[e.target]=true; }
				});
				...
				edgeEls.forEach(function(eg){
					var s = parseInt(eg.dataset.source), t = parseInt(eg.dataset.target);
		...
		svg.addEventListener('click', function(e){ if(!e.target.closest('.graph-node')) clearHL(); });
	}

	document.getElementById('footer-ts').textContent = new Date().toLocaleString();
	document.getElementById('footer-stats').textContent = 'Schema v'+report.version+' \u00b7 '+report.event_count+' events \u00b7 '+report.step_count+' steps';
...
// workflow report. The output embeds all CSS and JavaScript inline — no
// external dependencies, no network requests. Report data is injected via
// <script type="application/json"> tags (never parsed as HTML), and dynamic
// content in the JavaScript is escaped via the esc() function.
//
//nolint:funlen,varnamelen // HTML assembly is inherently a long sequence of writes.
...
<header>
  <div class="header-left">
    <h1><span class="logo-dot"></span>workflow-auditlog<span class="version">v`)
	sb.WriteString(report.Version)
	sb.WriteString(`</span></h1>
    <p class="subtitle">Workflow <span class="mono" id="workflow-id"></span>`)
	sb.WriteString(
		` &middot; Run <span class="mono" id="run-id"></span> &mdash; exported <span id="exported-at"></span></p>
  </div>
...
<script type="application/json" id="type-metadata">`)
