...
- **CI hardening** — `.github/workflows/ci.yml` and `website.yml` rewritten: all actions SHA-pinned, Go pinned to 1.26.5 with `GOTOOLCHAIN=go1.26.5`, `golangci-lint config verify` step, `stale-generation` job, `pnpm audit` in website CI, Firebase secret JSON validation via `node -e JSON.parse`.
...
- **`WithFlushInterval(d time.Duration)`** — bounded-latency NDJSON streaming option. Flushes the internal buffer at most once per interval, letting callers tune the latency/throughput trade-off between per-event `WithAutoFlush` and buffer-only `WithBufferSize`. `WithAutoFlush` takes precedence when both are set.
- **`StreamEvents(reader, validate, fn)`** — streaming NDJSON reader that invokes a per-event callback without materializing the full event slice. For multi-hour workflows producing 10k+ events where buffering the entire result would exhaust RAM. Matches `ReadEvents` error semantics (`ErrEmpty`, `ErrNoEvents`, `ErrOversizedLine`) and accepts an optional validate callback.
- **`MultiWriter`** — fans each event out to multiple `OnEvent`-style callbacks. Uses the exact `func(Event)` signature so it wires directly as `Config.OnEvent` and composes with `NDJSONStreamer.OnEvent` / `hub.OnEvent` without adapter lambdas. Thread-safe, nil-receiver-safe, skips nil callbacks.
- **`FailureReason` enum** — structured failure classification on `Event` (`failure_reason` JSON field). Three values the recorder can observe at the `AfterStep` callback level: `FailureReasonTimeout` (wraps `context.DeadlineExceeded`), `FailureReasonCanceled` (wraps `context.Canceled`), `FailureReasonUserError` (any other non-nil error). Predicate methods `Event.IsTimeout()`, `Event.IsCanceled()`, `Event.IsUserError()`. Zero value = unclassified (success or no error).
- **Extended `Diff()`** — `DiffResult` now carries aggregate deltas beyond step-level changes: `CriticalPathDeltaMs` (critical-path duration difference), `PeakConcurrencyDelta` (peak concurrent step count difference), and `CriticalPathStepsAdded`/`CriticalPathStepsRemoved` (set difference of critical-path membership). `HasChanges()` reflects all new fields.
- **Workflow-level retry/timeout surfacing** — six new `WorkflowReport` methods that answer whole-workflow questions without step-level iteration: `RetriedStepCount()`, `TotalRetryAttempts()`, `TimedOutSteps()`, `TimedOutStepCount()`, `HasWorkflowRetries()`, `HasWorkflowTimeouts()`.
